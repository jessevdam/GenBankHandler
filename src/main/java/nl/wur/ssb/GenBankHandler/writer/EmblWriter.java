...
public class EmblWriter extends InsdcWriter {
	...
	public void _write_the_first_line(Record record) throws Exception {
		...
		// 7. Sequence length
		this.writeSingleLine("ID", String.format("%s; %s; %s; %s; %s; %s; %s %s.", acc.getId(), subVersion, circular,
				strandType, dataclass, taxDivision, "" + record.size, residueType), false);
		write("XX\n");
		String accession = "";
	...
	public void writeRecord(Record record) throws Exception {
		...
		for (Reference reference : record.references) {
			/*
			 * RN - reference number (>=1 per entry) RC - reference comment (>=0
			 * per entry) RP - reference positions (>=1 per entry) RX -
			 * reference cross-reference (>=0 per entry) RG - reference group
			 * (>=0 per entry) RA - reference author(s) (>=0 per entry) RT -
			 * reference title (>=1 per entry) RL - reference location (>=1 per
			 * entry)
			 */
			if (reference.id != -1)
			...
			this.writeMultiLine("RT",
					(reference.title != null && !reference.title.equals("")) ? "\"" + reference.title + "\";" : ";",
					true);
